...
\begin{figure}
    ...
    \note{注：每个灰色区域代表一块独立的电路板。Geometry Board 为负责顶点变换的电路板，其中包含有命令处理器、几何引擎。Raster Memory Board 为光栅化电路板，其负责接收三角形，并生成对应的片元，同时进行着色操作。Display Generator Board 负责连接显示器，从显存中读取图像，并负责显示信号的生成。}
