...
{\added 本章提出了数据驱动的着色器程序性能预测方法。本章在引言部分，提出了着色器程序性能预测面临的诸多挑战，并在之后一节系统阐述了本章提出方法的针对性的设计思路。然后，本章详细介绍了该着色器程序性能预测方法的实现，包括指令计数跟踪和模型构建两个大任务下的诸多细节和子模块。在这之后，本章给出了基线模型和相关讨论，并分别给出了基线比较、挑战样本集基线比较、指令计数信息的消融实验、指令计数嵌入的对比试验，以及性能样本时间归一化和损失函数设计的相关实验。}